  ...
  try {
    ...
    const result = await pool.query(
      `SELECT h.household_id, h.household_no, h.householder_name, h.address,
              h.water_supply, h.electricity_connection, h.status, h.created_at
       FROM household h
...
// ✅ Update household status – GN only (verify / reject)
router.patch("/:id/status", requireAuth, requireRole("GN"), async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE household SET status=$1 WHERE household_id=$2 RETURNING household_id, status`,
      [status, id]
    );
