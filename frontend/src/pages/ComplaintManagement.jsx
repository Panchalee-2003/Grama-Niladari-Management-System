...
function IconUsers() {
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
      <path
        d="M16 11c1.66 0 3-1.34 3-3s-1.34-3-3-3-3 1.34-3 3 1.34 3 3 3ZM8 11c1.66 0 3-1.34 3-3S9.66 5 8 5 5 6.34 5 8s1.34 3 3 3ZM8 13c-2.67 0-8 1.34-8 4v2h12v-2c0-2.66-5.33-4-4-4Zm8 0c-.34 0-.7.02-1.07.05 1.16.84 2.07 1.97 2.07 3.45v2h7v-2c0-2.66-5.33-4-8-4Z"
        fill="#0B1B12"
      />
    </svg>
  );
...
function IconHand() {
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
      <path
        d="M7 22h10c2 0 3-1 3-3v-7c0-1.1-.9-2-2-2h-1V6a2 2 0 1 0-4 0v3H11V5a2 2 0 1 0-4 0v9H6c-1.1 0-2 .9-2 2v1c0 3 2 5 3 5Z"
        fill="#0B1B12"
        opacity="0.9"
      />
    </svg>
...
export default function ComplaintManagement() {
  ...
  const fetchComplaints = useCallback(async (filterTab) => {
    ...
    try {
      const params =
        filterTab && filterTab !== "ALL" ? `?status=${filterTab}` : "";
  ...
  const handleStatusChange = async (newStatus) => {
    ...
    try {
      const r = await api.patch(
        `/api/complaint/${modalDetail.complaint.complaint_id}/status`,
        { status: newStatus },
      );
      if (r.data.ok) {
        setModalDetail((prev) => ({
          ...prev,
          complaint: { ...prev.complaint, status: newStatus },
        }));
        setComplaints((prev) =>
          prev.map((c) =>
            c.complaint_id === modalDetail.complaint.complaint_id
              ? { ...c, status: newStatus }
              : c,
          ),
        );
  ...
  const handleAddResponse = async (e) => {
    ...
    try {
      const r = await api.post(
        `/api/complaint/${modalDetail.complaint.complaint_id}/response`,
        { message: responseMsg },
      );
      if (r.data.ok) {
        setModalDetail((prev) => ({
          ...prev,
          responses: [...prev.responses, r.data.response],
        }));
        ...
        setComplaints((prev) =>
          prev.map((c) =>
            c.complaint_id === modalDetail.complaint.complaint_id
              ? {
                  ...c,
                  response_count: c.response_count + 1,
    ...
    <div className="cmp-wrap">
      ...
      <aside className="cmp-side">
        ...
        <nav className="cmp-nav">
          <Link className="cmp-item" to="/gn">
            <span className="cmp-i">
              <IconDash />
            </span>
            Dashboard
          </Link>
          <Link className="cmp-item" to="/gn-households">
            <span className="cmp-i">
              <IconUsers />
            </span>
            Households
          </Link>
          <Link className="cmp-item" to="/gn-certificates">
            <span className="cmp-i">
              <IconDoc />
            </span>
            Certificates
          ...
          <Link className="cmp-item" to="/gn-notices">
            <span className="cmp-i">
              <IconBell />
            </span>
            Notices
          </Link>
          <Link className="cmp-item" to="/gn-allowances">
            <span className="cmp-i">
              <IconHand />
            </span>
            Allowances &amp; Aids
        ...
        <div className="cmp-content">
          ...
          <div className="cmp-tabs">
            {TABS.map((t) => (
              <div
                key={t}
                className={`cmp-tab ${tab === t ? "cmp-tab-active" : ""}`}
                onClick={() => setTab(t)}
              >
                ...
                {complaints.map((c) => {
                  ...
                  return (
                    <div className="cmp-row" key={c.complaint_id}>
                      ...
                      <div className="cmp-muted">
                        {formatDate(c.created_at)}
                      </div>
                      <div>
                        <span className={`cmp-pill ${s.cls}`}>{s.label}</span>
                      </div>
      ...
      {modal && (
        <div className="cmp-modal-overlay" onClick={closeModal}>
          <div className="cmp-modal" onClick={(e) => e.stopPropagation()}>
            <button className="cmp-modal-close" onClick={closeModal}>
              ✕
            </button>
            ...
            {modalDetail && (
              <>
                ...
                <div className="cmp-modal-header">
                  <div>
                    <h2 className="cmp-modal-title">
                      {modalDetail.complaint.subject}
                    </h2>
                    <div className="cmp-modal-meta">
                      ...
                      <span>•</span>
                      <span>
                        {formatDate(modalDetail.complaint.created_at)}
                      </span>
                    </div>
                  </div>
                  ...
                  {modalDetail.responses.map((r) => (
                    <div key={r.response_id} className="cmp-response-bubble">
                      <div className="cmp-response-meta">
                        <span className="cmp-response-author">
                          {r.gn_name || "GN Officer"}
                        </span>
                        <span className="cmp-response-date">
                ...
                {/* Add Response Form */}
                <form
                  className="cmp-response-form"
                  onSubmit={handleAddResponse}
                >
                  <label className="cmp-response-form-label">
                    Add Response:
                  </label>
                  {responseError && (
                    <div
