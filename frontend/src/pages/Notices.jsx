    ...
    <div className="nt-page">
      ...
      <nav className="cd-nav">
        ...
        <Link className="cd-nav-item" to="/household">
          <IconUser />
          <span>{t("nav.household") || "Household"}</span>
        </Link>
        <Link className="cd-nav-item" to="/certificates">
          <IconDoc />
          <span>{t("nav.certificates") || "Certificates"}</span>
        </Link>
        ...
        <Link className="cd-nav-item" to="/availability">
          <IconCalendar />
          <span>{t("nav.gnSchedule") || "GN Schedule"}</span>
        </Link>
      </nav>
      ...
      <main className="nt-main">
        ...
        <p className="nt-page-sub">
          Official announcements and news from the Maspanna Grama Niladhari
          Division
        </p>
              ...
              <div className="nt-card" key={n.notice_id}>
                ...
                <div className="nt-card-body">
                  <div className="nt-card-date">
                    ...
                    {n.gn_name && (
                      <span className="nt-card-author"> · {n.gn_name}</span>
                    )}
                  </div>
                  <h2 className="nt-card-title">{n.title}</h2>
                  {n.description && (
                    <p
                      className={`nt-card-desc ${expanded === n.notice_id ? "nt-expanded" : "nt-collapsed"}`}
                    >
                      {n.description}
                    </p>
                  )}
                  {n.description && n.description.length > 150 && (
                    <button
                      ...
                      onClick={() =>
                        setExpanded(
                          expanded === n.notice_id ? null : n.notice_id,
                        )
                      }
                    >
